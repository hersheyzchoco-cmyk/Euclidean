...
local function safeGetRF(svc, name)
...
local function wh_requestFunc(options)
...
local function getTargetPriorityValue(npcModel, nameLower, prio1, prio2)
...
local function calculateCrystalTargetCFrame(targetEnemy, targetPart)
    ...
    dlf_moveConnection = RunService.Stepped:Connect(function(_, dt)
        ...
        elseif target and target.part then
            ...
            if mode == "Dungeon" then
                ...
                if hoverCF then
                    local hoverPos = hoverCF.Position + Vector3.new(0, math.max(12, dl_farm_height + 4), 0)
...
function forceResetAndReextract(reason)
    ...
    if extracted and #extracted > 0 then
        ...
        if firstRoom then
            ...
            if cf then
                dl_directTeleport(CFrame.new(cf.Position + Vector3.new(0, dl_farm_height + 4, 0), cf.Position))
...
function dl_processLootRoom(lootRoomEntry)
    ...
    dl_cancelTween()
    local enterPos = cachedLootRoomCF.Position + Vector3.new(0, dl_farm_height + 4, 0)
...
function dl_processLockedRoom(lockedRoomEntry)
    ...
    dl_cancelTween()
    local enterPos = cachedLockedRoomCF.Position + Vector3.new(0, dl_farm_height + 4, 0)
                            ...
                            if roomModel then
                                ...
                                if not endlessContinuePending then
                                    ...
                                    if cachedRoomCF then
                                        local enterPos = cachedRoomCF.Position + Vector3.new(0, dl_farm_height + 4, 0)
...
local function getSummonSlotData()
...
local BUFF_DEFINITIONS = {
    { Title = "Overwhelming Force", Desc = "+Overall Damage",   Key = "BuffPrio_Overwhelming_Force", Default = 14 },
    { Title = "Fleetfoot",          Desc = "+Movement Speed",   Key = "BuffPrio_Fleetfoot",          Default = 10 },
    { Title = "Ironhide",           Desc = "+Max HP",            Key = "BuffPrio_Ironhide",           Default = 8 },
    { Title = "Bloodlust",          Desc = "+Enemy Kill EXP",    Key = "BuffPrio_Bloodlust",          Default = 7 },
    { Title = "Scholar's Insight",  Desc = "+Completion EXP",    Key = "BuffPrio_Scholars_Insight",   Default = 7 },
    { Title = "Fortune",            Desc = "+Coins",             Key = "BuffPrio_Fortune",            Default = 6 },
    { Title = "Arcane Might",       Desc = "+Skill Damage",      Key = "BuffPrio_Arcane_Might",       Default = 13 },
    { Title = "Deadeye",            Desc = "+Crit Rate",         Key = "BuffPrio_Deadeye",            Default = 12 },
    { Title = "Focused Casting",    Desc = "+Skill Crit Rate",   Key = "BuffPrio_Focused_Casting",    Default = 11 },
    { Title = "Devastation",        Desc = "+Skill Crit Damage", Key = "BuffPrio_Devastation",        Default = 12 },
    { Title = "Bulwark",            Desc = "+Damage Reduction",  Key = "BuffPrio_Bulwark",            Default = 9 },
    { Title = "Steadfast",          Desc = "+Parry Window",      Key = "BuffPrio_Steadfast",          Default = 5 },
    { Title = "Nimble",             Desc = "-Dodge Cooldown",    Key = "BuffPrio_Nimble",             Default = 8 },
    { Title = "Flowstate",          Desc = "+Skill Cooldown",    Key = "BuffPrio_Flowstate",          Default = 13 },
}
...
do
    ...
    local MB = TabsLobby.Gear:AddRightGroupbox("Shop Merchant", "shopping-bag")
    MB:AddLabel("<b>SHOP BUYER</b>", true)
    MB:AddDivider()
    MB:AddToggle("AutoBuyMerchant", { Text = "Auto Purchase Shop Stock", Default = false })
    MB:AddDropdown("BuyRarities", {
        Values = { "Common", "Uncommon", "Rare", "Epic", "Legendary", "Mythic", "Celestial" },
        Default = {},
        Multi = true,
...
do
    local function makeTargetList(maxVal)
        local t = {}
...
local function isSkillOnCD(idx)
    local ok, res = pcall(function()
        local pg = LocalPlayer:FindFirstChild("PlayerGui")
        local main = pg and pg:FindFirstChild("Main")
        local hud = main and main:FindFirstChild("HUD")
...
local function dl_makeVec(x, y, z)
...
local function dl_getActiveMiniLavas()
    local list = {}
...
local function dl_getActiveMassiveWipes()
    local list = {}
    local telegraphsFolder = workspace:FindFirstChild("SpellTelegraphs")
    if not telegraphsFolder then return list end
...
local function dl_getArenaBoundary()
    local hrp = dl_getHRP()
...
local function dl_isPositionInLava(pos, miniLavas, buffer)
...
local function dl_resolveSafePosition(currentPos, miniLavas)
...
local function executeEndAction(actionType, mode)
...
do
    ...
    PerfGroup:AddToggle("PotatoMode", { Text = "Disable 3D Rendering", Default = false, Callback = function(v) RunService:Set3dRenderingEnabled(not v) end })
